...
function WinnerIcon({ winner }) {
  if (winner === 'flowable') return <CheckCircle2 size={15} className="text-emerald-400 shrink-0 mt-0.5" />
  if (winner === 'newgen')   return <CheckCircle2 size={15} className="text-blue-400 shrink-0 mt-0.5" />
  return <MinusCircle size={15} className="text-gray-500 shrink-0 mt-0.5" />
}

function CapRow({ cap, idx }) {
  return (
    <tr className={clsx('border-b border-gray-800/60', idx % 2 === 0 ? 'bg-gray-900/40' : '')}>
      <td className="py-3 px-4">
        <div className="font-medium text-gray-200 text-sm">{cap.feature}</div>
      </td>
      <td className="py-3 px-4">
        <div className="flex items-start gap-2">
          {cap.winner === 'flowable'
            ...
            : cap.winner === 'both'
            ? <MinusCircle size={14} className="text-gray-500 shrink-0 mt-0.5" />
            : <XCircle size={14} className="text-red-400 shrink-0 mt-0.5" />}
          <span className={clsx('text-xs', cap.winner === 'flowable' ? 'text-emerald-300' : 'text-gray-400')}>
            {cap.flowable}
          </span>
        </div>
      </td>
      <td className="py-3 px-4">
        <div className="flex items-start gap-2">
          {cap.winner === 'newgen'
            ...
            ? <MinusCircle size={14} className="text-gray-500 shrink-0 mt-0.5" />
            : <XCircle size={14} className="text-red-400 shrink-0 mt-0.5" />}
          <span className={clsx('text-xs', cap.winner === 'newgen' ? 'text-blue-300' : 'text-gray-500')}>
            {cap.newgen}
          </span>
        </div>
      </td>
      <td className="py-3 px-4 text-center">
        {cap.winner === 'flowable' && <span className="text-xs font-bold text-emerald-400 bg-emerald-900/30 px-2 py-0.5 rounded">Flowable</span>}
        {cap.winner === 'newgen'   && <span className="text-xs font-bold text-blue-400 bg-blue-900/30 px-2 py-0.5 rounded">Newgen</span>}
        {cap.winner === 'both'     && <span className="text-xs text-gray-600">Equal</span>}
      </td>
    </tr>
  )
}
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="card text-center border-emerald-800/50 border">
          <div className="text-3xl font-bold text-emerald-400">{flowableWins}</div>
          <div className="text-sm text-gray-400 mt-1">Flowable Advantages</div>
        </div>
        <div className="card text-center border-gray-700/50 border">
          <div className="text-3xl font-bold text-gray-400">{both}</div>
          <div className="text-sm text-gray-400 mt-1">Equal / Both Good</div>
        </div>
        <div className="card text-center border-blue-800/50 border">
          <div className="text-3xl font-bold text-blue-400">{newgenWins}</div>
          <div className="text-sm text-gray-400 mt-1">Newgen Advantages</div>
        </div>
      </div>

      {/* Process definition info */}
      {procDef && !procDef.error && (
        <div className="p-4 bg-emerald-900/10 border border-emerald-900/30 rounded-xl text-sm">
          <div className="font-semibold text-emerald-400 mb-1">Live Process Definition</div>
          <div className="grid grid-cols-4 gap-4 text-xs text-gray-400">
            <div><span className="text-gray-600">Name: </span>{procDef.name}</div>
            <div><span className="text-gray-600">Version: </span><span className="text-white font-bold">v{procDef.version}</span></div>
            <div><span className="text-gray-600">Deployment: </span>{procDef.deploymentId}</div>
            <div><span className="text-gray-600">Resource: </span>{procDef.resourceName}</div>
          </div>
      ...
      {loading ? (
        ...
        <div className="card p-0 overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="bg-gray-800/80 text-xs text-gray-400 border-b border-gray-700">
                <th className="text-left py-3 px-4 font-semibold w-1/4">Feature</th>
                <th className="text-left py-3 px-4 font-semibold w-[37%]">
                  <span className="text-emerald-400">Flowable BPM</span>
                </th>
                <th className="text-left py-3 px-4 font-semibold w-[37%]">
                  <span className="text-blue-400">Newgen iBPS</span>
                </th>
      ...
      <div>
        <h2 className="text-lg font-bold text-white mb-4">Architecture Highlights</h2>
      ...
      {/* Bottom CTA */}
      <div className="p-5 bg-gradient-to-r from-rakbank-red/20 to-transparent border border-rakbank-red/30 rounded-xl">
        <div className="font-bold text-white text-lg mb-1">Why Flowable for RAKBANK?</div>
        ...
        <div className="flex flex-wrap gap-2">
          {['Zero License Cost', 'BPMN 2.0 Standard', 'Hot Deployment', 'Embedded in Spring Boot',
            ...
            <span key={t} className="text-xs bg-gray-800 text-gray-300 px-2.5 py-1 rounded-full border border-gray-700">
              {t}
            </span>
          ))}
        </div>
