...
/* Maps BPMN activity IDs to display metadata */
const STEP_META = {
...
function StepIcon({ status }) {
  ...
  if (status === 'IN_PROGRESS') return <Loader2 size={16} className="text-blue-400 animate-spin" />
...
function VarRow({ k, v }) {
  ...
  const color   = colorMap[display] || 'text-gray-300'
  return (
...
function OutcomeBanner({ app }) {
  ...
  if (app.status === 'APPROVED') {
    return (
      <div className="flex items-center gap-3 p-4 bg-emerald-900/30 border border-emerald-800 rounded-xl">
        <CheckCircle2 size={24} className="text-emerald-400 shrink-0" />
        <div>
          <div className="font-semibold text-emerald-300">Account Successfully Opened</div>
  ...
  if (app.status === 'REJECTED') {
    return (
      <div className="flex items-center gap-3 p-4 bg-red-900/30 border border-red-800 rounded-xl">
        ...
        <div>
          <div className="font-semibold text-red-300">Application Rejected</div>
  ...
  if (app.status === 'RUNNING') {
    return (
      <div className="flex items-center gap-3 p-4 bg-blue-900/20 border border-blue-900/50 rounded-xl">
        <Loader2 size={20} className="text-blue-400 animate-spin shrink-0" />
        <div>
          <div className="font-semibold text-blue-300">Processing</div>
...
export default function ApplicationDetail() {
  ...
  if (loading) {
    return <div className="p-8 text-gray-600 flex items-center gap-2"><Loader2 className="animate-spin" size={18}/> Loading...</div>
  }
  if (!app) {
    return <div className="p-8 text-gray-600">Application not found.</div>
  }

  // Build ordered unique step list, merging parallel group
  ...
  return (
    <div className="p-6 space-y-5 max-w-4xl">
      {/* Back */}
      <button onClick={() => navigate('/')} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-300">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">{app.companyName}</h1>
          <div className="text-xs text-gray-500 mt-1 font-mono">{app.businessKey} · {app.applicationId}</div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="col-span-2 card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold text-gray-300">Process Timeline</h2>
          </div>
          ...
          <div className="space-y-1">
            {otherSteps.map(step => {
              ...
              // Insert parallel group after screeningFork
              ...
              return (
                <div key={step.id}>
                  <div className={clsx(
                    ...
                    step.status === 'IN_PROGRESS' ? 'bg-blue-900/20 border border-blue-900/40' :
                    step.status === 'COMPLETED'   ? 'hover:bg-gray-800/30' : 'opacity-40'
                  )}>
                    <StepIcon status={step.status} />
                    <span className="text-base">{meta.icon}</span>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-gray-200">{meta.label || step.name}</div>
                      {step.durationMs && (
                        <div className="text-xs text-gray-600">{step.durationMs}ms</div>
                      )}
                    </div>
                  ...
                  {insertParallel && (
                    <div className="ml-8 my-1 border-l-2 border-dashed border-gray-700 pl-4 space-y-1">
                      ...
                      {parallelSteps.map(ps => {
                        ...
                        return (
                          <div key={ps.id} className={clsx(
                            ...
                            ps.status === 'IN_PROGRESS' ? 'bg-blue-900/20' :
                            ps.status === 'COMPLETED'   ? 'bg-gray-800/30' : 'opacity-40'
                          )}>
                            <StepIcon status={ps.status} />
                            <span className="text-base">{pm.icon}</span>
                            <div className="flex-1">
                              <div className="text-sm text-gray-200">{pm.label}</div>
                              {ps.durationMs && <div className="text-xs text-gray-600">{ps.durationMs}ms</div>}
                            </div>
                          </div>
                        )
                      })}
        ...
        {/* Variables Panel */}
        <div className="card">
          <h2 className="text-sm font-semibold text-gray-300 mb-3">Process Variables</h2>
